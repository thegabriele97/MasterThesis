...
int main(void) {
	...
	XTmrCtr_SetResetValue(&htimer1, 1, 0x0); // Setting Timer0[0] reset value at 0x0 (init value when timer is resetted or started)
	XTmrCtr_SetOptions(&htimer1, 1, XTC_AUTO_RELOAD_OPTION); // Ensure that Timer0[0] always counts even when overflows

	xPortInstallInterruptHandler(XPAR_INTC_0_TMRCTR_1_VEC_ID, ISRTimer1, NULL);
	XTmrCtr_SetResetValue(&htimer1, 0, XPAR_TMRCTR_1_CLOCK_FREQ_HZ);
	XTmrCtr_SetOptions(&htimer1, 0, XTC_INT_MODE_OPTION | XTC_AUTO_RELOAD_OPTION | XTC_DOWN_COUNT_OPTION);
	XTmrCtr_SetHandler(&htimer1, (void *)ISRTimer1, NULL);

	XTmrCtr_Start(&htimer1, 1);
	XTmrCtr_Start(&htimer1, 0);
	xStart = XTmrCtr_GetValue(&htimer1, 1);
#endif
